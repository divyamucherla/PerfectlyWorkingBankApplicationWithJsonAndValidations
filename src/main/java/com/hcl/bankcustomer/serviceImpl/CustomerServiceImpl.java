...
public class CustomerServiceImpl implements CustomerService {
	...
	public static List<Customer> json() throws JsonParseException, JsonMappingException, IOException {
		...
		InputStream input = TypeReference.class.getResourceAsStream("/static/Details.json");
...
//		return li;
		...
		for (int i = 0; i < jsonList.size(); i++) {
			...
			if (customer.getId() == id) {
				...
				if(customer.getAddress()!=address && address!=null) {
				customer.setAddress(address);
				}
				if(customer.getName()!=name && name!=null) {
				customer.setName(name);
				}
				if(customer.getRole()!=role && role!=null) {
